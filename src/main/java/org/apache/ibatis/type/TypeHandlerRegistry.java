...
public final class TypeHandlerRegistry {
     ...
     *     ****
     * }
     * 在TypeHandlerRegistry新建的时候即将JdbcType 类型和处理器进行了注册
     */
    private final Map<JdbcType, TypeHandler<?>> JDBC_TYPE_HANDLER_MAP = new EnumMap<JdbcType, TypeHandler<?>>(JdbcType.class);
    private final Map<Type, Map<JdbcType, TypeHandler<?>>> TYPE_HANDLER_MAP = new ConcurrentHashMap<Type, Map<JdbcType, TypeHandler<?>>>();
    private final TypeHandler<Object> UNKNOWN_TYPE_HANDLER = new UnknownTypeHandler(this);
    /**
     * 存储所有TYPE_HANDLERS ，key 为handler的class对象 、value 为具体的handler对象
     */
    ...
    /**
     *  类型处理器注册过程的终点 通用map 里面key 为javaType，value为 map <jdbcType,handler>
     * @param javaType key
     * @param jdbcType value.key
     * @param handler value.value
     */
